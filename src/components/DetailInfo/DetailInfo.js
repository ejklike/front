...
class DetailImg extends React.Component {
  ...
  getImgSrc(photos){
    ...
    if (!photos){
      ...
      let request = {'maxWidth': 280, 'maxHeight': 250};
      vvm.props.photos[0].getUrl(request, function(response, status){
        console.log("response", response, "status", status);
      })
      })
      let vm = this;
      
      Promise.all([imgPromise]).then(function(results){
        vm.setState({
          imgSrc: results[0]
        });
      });
...
class DetailInfo extends React.Component {
  ...
  render() {
    ...
    if (Object.keys(this.props.detail).length !== 0 || this.props.detail.constructor !== Object){
      console.log("detail",this.props.detail);  
      console.log("aaa", typeof this.props.detail.photos !== 'undefined'? this.props.detail.photos[0].getUrl({'maxWidth': 280, 'maxHeight': 200}) : noImg);
      return(
        <div>
          <div>
          ...
          <div id="Reviews">
            {this.props.detail.reviews &&
              this.props.detail.reviews.map((review,i) => {
                return(
